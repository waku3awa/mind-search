...
from mindsearch.agent.mindsearch_prompt import (GRAPH_PROMPT_JP, FINAL_RESPONSE_JP, graph_fewshot_example_jp, fewshot_example_jp, searcher_input_template_jp, searcher_context_template_jp, searcher_system_prompt_jp)
...
def init_agent(lang='cn', model_format='internlm_server',search_engine='DuckDuckGoSearch'):
    ...
    interpreter_prompt = GRAPH_PROMPT_CN if lang == 'cn' else (GRAPH_PROMPT_EN if lang == 'en' else GRAPH_PROMPT_JP)
    plugin_prompt = searcher_system_prompt_cn if lang == 'cn' else (searcher_system_prompt_en if lang == 'en' else searcher_system_prompt_jp)
    if not model_format.lower().startswith('internlm'):
        interpreter_prompt += graph_fewshot_example_cn if lang == 'cn' else (graph_fewshot_example_en if lang == 'en' else graph_fewshot_example_jp)
        plugin_prompt += fewshot_example_cn if lang == 'cn' else (fewshot_example_en if lang == 'en' else fewshot_example_jp)
    ...
    agent = MindSearchAgent(
        ...
        protocol=MindSearchProtocol(meta_prompt=datetime.now().strftime(
            'The current date is %Y-%m-%d.'),
                                    ...
                                    if lang == 'cn' else (FINAL_RESPONSE_EN if lang == 'en' else FINAL_RESPONSE_JP)),
        searcher_cfg=dict(
            llm=llm,
            ...
            template=dict(input=searcher_input_template_cn
                          if lang == 'cn' else (searcher_input_template_en if lang == 'en' else searcher_input_template_jp),
                          context=searcher_context_template_cn
                          if lang == 'cn' else (searcher_context_template_en if lang == 'en' else searcher_context_template_jp))),
        max_turn=10)
    return agent
